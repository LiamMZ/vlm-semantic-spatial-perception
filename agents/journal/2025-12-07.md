### 2025-12-07-1858 – Task analyzer prompt externalization

- **Owner**: AI Agent (prompt refactor)
- **Agents affected**: LLMTaskAnalyzer, planning docs
- **Git commit(s)**: Based on 32293f7180c50bd0b61b3a4168141e9faeb68b5e
- **Context**: Migrated the task analyzer off its inline prompt while merging the stricter PDDL/global predicate guidance into the YAML-driven template.
- **Actions**:
  - Routed analysis prompt construction through `config/llm_task_analyzer_prompts.yaml` with template rendering for task, robot description, objects, and relationships.
  - Expanded the YAML prompt to request the full TaskAnalysis payload and reinforced PDDL formatting/global predicate rules.
- **Result**: Analyzer now consistently uses the external template for all runs and the config encodes the richer schema plus the inline safety checks.
- **References**: `src/planning/llm_task_analyzer.py`, `config/llm_task_analyzer_prompts.yaml`, `docs/planning.md`
...
### 2025-12-07-1741 – Affordance-embedded interaction points

- **Owner**: AI Agent (prompt hardening)
- **Agents affected**: ObjectTracker prompts, perception docs
- **Git commit(s)**: Based on b328f4b38c59a9cb92fd92a25d0a9261a1a45c01
- **Context**: Requested embedding interaction points inside the affordances list rather than a separate field, and to keep parsing minimal.
- **Actions**: Updated the analysis response schema to make `affordances` a list of `{affordance, position, reasoning}` entries, simplified the ObjectTracker parser to consume that shape directly, and refreshed prompt docs.
- **Result**: Analysis outputs now bundle interaction points with their affordances in a single array and the tracker parses only that structure.
- **References**: `config/object_tracker_prompts.yaml`, `src/perception/object_tracker.py`, `agents/design/prompts_configuration.md`, `docs/perception.md`
...
### 2025-12-07-1907 – Untyped analyzer prompt trim

- **Owner**: AI Agent (prompt hardening)
- **Agents affected**: LLMTaskAnalyzer, planning docs, domain maintainer
- **Git commit(s)**: Based on 32293f7180c50bd0b61b3a4168141e9faeb68b5e
- **Context**: The task analyzer prompt still requested unused fields and suggested typed PDDL snippets; we need it untyped and aligned with what the code consumes.
- **Actions**:
  - Removed unused fields from the analyzer prompt schema and prohibited type annotations across predicates, actions, and quantifiers.
  - Adjusted action parameter parsing to accept untyped variables while defaulting types internally.
  - Updated planning docs to reflect the leaner, untyped prompt contract.
- **Result**: Task analysis now asks only for the fields used in PDDL construction, enforces untyped outputs, and the maintainer can ingest untyped action parameters.
- **References**: `config/llm_task_analyzer_prompts.yaml`, `src/planning/pddl_domain_maintainer.py`, `docs/planning.md`

### 2025-12-07-1929 – LMH prompt grounding on goal objects

- **Owner**: AI Agent (prompt alignment)
- **Agents affected**: LLMTaskAnalyzer prompts (LMH), planning docs
- **Git commit(s)**: Based on 32293f7180c50bd0b61b3a4168141e9faeb68b5e
- **Context**: The LMH analyzer prompt needed to match the updated untyped template while grounding goals on observed goal objects rather than quantified predicates.
- **Actions**:
  - Replaced the LMH prompt with the latest analyzer template and instructed goal_predicates to be literals over goal_objects (observed IDs).
  - Documented the LMH variant in planning docs.
- **Result**: Both prompt variants are aligned; the LMH set now returns concrete goal literals tied to detected goal objects.
- **References**: `config/llm_task_analyzer_prompts_lmh.yaml`, `docs/planning.md`

### 2025-12-07-2016 – Restore predicate emission in perception

- **Owner**: AI Agent (bug fix)
- **Agents affected**: ObjectTracker prompts, perception docs
- **Git commit(s)**: Based on f0b887d396d0c4f4038d95ac51dab4e5e20cfd91
- **Context**: Recent PDDL problems were missing all observed predicates; the analysis response schema no longer exposed a `predicates` field so the VLM responses dropped them and the registry stayed empty.
- **Actions**:
  - Added a `predicates` string array to `analysis.response_schema` in `config/object_tracker_prompts.yaml`.
  - Updated prompt configuration and perception docs to call out the explicit predicates array in the enforced schema.
- **Result**: Analysis responses can emit predicate strings again, letting the registry populate the PDDL initial state.
- **References**: `config/object_tracker_prompts.yaml`, `agents/design/prompts_configuration.md`, `docs/perception.md`
