...
- `analysis.response_schema` – structured JSON contract enforced via `response_json_schema`; requires `object_type` and `position`, allows `affordances` as a list of `{affordance, position, reasoning}` entries, and now explicitly includes a `predicates` array for PDDL strings.
