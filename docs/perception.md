...
- Analysis prompts now use structured generation with a JSON schema enforced via `response_json_schema`; the schema requires `object_type` and `position` (normalized [y, x]) and allows `affordances` as a list of `{affordance, position, reasoning}` entries plus optional `predicates`.
