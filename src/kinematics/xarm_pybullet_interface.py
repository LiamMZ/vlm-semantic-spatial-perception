"""
xArm7 PyBullet Simulation Interface

Drop-in replacement for CuRoboMotionPlanner when running without a physical robot
or GPU/CUDA dependencies. Uses PyBullet for forward kinematics and transform
computation via the URDF shipped in src/kinematics/sim/.
...
# Path to the URDF bundled with this project
_SIM_DIR = Path(__file__).parent / "sim"
_DEFAULT_URDF = _SIM_DIR / "xarm7.urdf"

# Default camera link name in the xArm7 URDF (wrist-mounted RealSense)
_DEFAULT_CAMERA_LINK = "camera_color_optical_frame"
# TCP / end-effector link name
_DEFAULT_TCP_LINK = "link_tcp"
# Base link
_BASE_LINK = "link_base"
# Home configuration (radians) — all joints at zero
_HOME_JOINTS = np.zeros(7, dtype=np.float64)


class XArmPybulletInterface:
    """
    xArm7 kinematics via PyBullet DIRECT mode.
    ...
    def __init__(
        ...
        camera_link_name: str = _DEFAULT_CAMERA_LINK,
        tcp_link_name: str = _DEFAULT_TCP_LINK,
        ...
        self._joints = _HOME_JOINTS.copy()
        ...
        self._init_pybullet()
        logger.info("XArmPybulletInterface ready (URDF=%s)", self.urdf_path)
    ...
    def get_robot_tcp_pose(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        try:
            ...
            base_idx = self._get_link_index(_BASE_LINK)
...
def create_sim_interface(
    camera_link: str = _DEFAULT_CAMERA_LINK,
    static_camera_tf: Optional[Any] = None,
) -> XArmPybulletInterface:
    """Convenience factory using the bundled xArm7 URDF."""
